//+------------------------------------------------------------------+
//|                   SYSTEM2_NAS_V4_BreakoutRetest_EA.mq5          |
//|               Aggressive Breakout + Retest EA (NAS100)          |
//+------------------------------------------------------------------+
#property strict
#property version   "4.00"
#property description "System 2 breakout retest EA for NAS100 / US100 on M5"
#property description "Separate input signature from FTMO EA"

input group "=== SESSION / RANGE (SERVER TIME) ==="
input int    S2_InpRangeStartHour        = 14;
input int    S2_InpRangeStartMinute      = 30;
input int    S2_InpRangeEndHour          = 15;
input int    S2_InpRangeEndMinute        = 0;
input int    S2_InpTradeEndHour          = 20;
input int    S2_InpTradeEndMinute        = 0;
input bool   S2_InpCloseAtSessionEnd     = false;

input group "=== BREAKOUT / RETEST RULES ==="
input double S2_InpBreakoutBufferPoints  = 20.0;   // smaller = more setups
input double S2_InpRetestTolerancePoints = 40.0;   // how close price can come back to breakout level
input int    S2_InpMaxBarsAfterBreakout  = 5;      // retest must happen within this many bars
input double S2_InpMinBodyPct            = 20.0;   // reclaim candle quality
input bool   S2_InpRequirePrevBreak      = false;  // aggressive mode defaults false
input int    S2_InpMaxTradesPerDay       = 2;
input int    S2_InpCooldownBars          = 0;

input group "=== STOP / TARGET ==="
input int    S2_InpATRPeriod             = 14;
input double S2_InpStopATRMultiplier     = 0.8;
input double S2_InpRRRatio               = 2.0;
input double S2_InpMaxSLPoints           = 5000.0;
input double S2_InpMinRangePoints        = 80.0;   // allow more days
input double S2_InpMaxRangePoints        = 3000.0;

input group "=== OPTIONAL TREND FILTER ==="
input bool   S2_InpUseTrendFilter        = true;
input int    S2_InpTrendFastEMA          = 20;
input int    S2_InpTrendSlowEMA          = 50;
input ENUM_TIMEFRAMES S2_InpTrendTF      = PERIOD_M15;

input group "=== RISK ==="
input double S2_InpRiskPct               = 1.0;
input double S2_InpStartBalanceOverride  = 0.0;
input double S2_InpMaxDailyLossPct       = 2.5;
input double S2_InpMaxTotalLossPct       = 8.0;
input double S2_InpProfitTargetPct       = 10.0;
input bool   S2_InpPermanentHalt         = false;

input group "=== EXECUTION ==="
input int    S2_InpMagic                 = 20250416;
input int    S2_InpSlippage              = 20;
...
int OnInit()
{
   g_atrHandle = iATR(_Symbol, PERIOD_M5, S2_InpATRPeriod);
   ...
   if(S2_InpUseTrendFilter)
   {
      g_trendFastHandle = iMA(_Symbol, S2_InpTrendTF, S2_InpTrendFastEMA, 0, MODE_EMA, PRICE_CLOSE);
      g_trendSlowHandle = iMA(_Symbol, S2_InpTrendTF, S2_InpTrendSlowEMA, 0, MODE_EMA, PRICE_CLOSE);
   ...
   g_startBalance = (S2_InpStartBalanceOverride > 0.0) ? S2_InpStartBalanceOverride : bal;
   ...
   PrintFormat("System2 Breakout Retest EA initialized | %s | Risk %.2f%% | RR 1:%.2f",
               _Symbol, S2_InpRiskPct, S2_InpRRRatio);
...
void OnTick()
{
   ...
   if(g_tradesToday >= S2_InpMaxTradesPerDay)
   {
...
int RangeStartMinutes() { return S2_InpRangeStartHour * 60 + S2_InpRangeStartMinute; }
int RangeEndMinutes()   { return S2_InpRangeEndHour   * 60 + S2_InpRangeEndMinute; }
int TradeEndMinutes()   { return S2_InpTradeEndHour   * 60 + S2_InpTradeEndMinute; }
...
void UpdateRange()
{
   ...
   if(now >= end && g_rangeBuilt && !g_rangeLocked)
   {
      ...
      g_rangeLocked = true;
      if(rangePts < S2_InpMinRangePoints || rangePts > S2_InpMaxRangePoints)
...
{
   if(!S2_InpCloseAtSessionEnd) return;
...
{
   if(!S2_InpUseTrendFilter) return true;
...
void ProcessRetestStateMachine()
{
   ...
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   double breakBuf = S2_InpBreakoutBufferPoints * point;
   double retestTol = S2_InpRetestTolerancePoints * point;
   ...
   {
      g_barsSinceBreakout++;
      if(g_barsSinceBreakout > S2_InpMaxBarsAfterBreakout)
      {
         g_cntExpired++;
         g_state = RS_IDLE;
         g_direction = 0;
      ...
      if(g_direction == 1)
      {
         ...
         if(retested)
         {
            ...
            bool brokePrev = S2_InpRequirePrevBreak ? (c > prevHigh) : true;
            bool bodyOK = (bodyPct >= S2_InpMinBodyPct);
      ...
      else if(g_direction == -1)
      {
         ...
         if(retested)
         {
            ...
            bool brokePrev = S2_InpRequirePrevBreak ? (c < prevLow) : true;
            bool bodyOK = (bodyPct >= S2_InpMinBodyPct);
...
double CalcLots(double entryPrice, double slPrice)
{
   ...
   double riskCash = balance * S2_InpRiskPct / 100.0;
...
bool ExecuteTrade(int direction, double signalHigh, double signalLow)
{
   ...
   if(direction == 1)
   {
      ...
      sl = baseLow - (atr * S2_InpStopATRMultiplier);
   }
   else
   {
      double baseHigh = (g_retestExtreme > 0.0) ? g_retestExtreme : signalHigh;
      sl = baseHigh + (atr * S2_InpStopATRMultiplier);
   }

   double slPts = MathAbs(entry - sl) / point;
   if(slPts > S2_InpMaxSLPoints)
   {
   ...
   double tp = (direction == 1)
               ? entry + (MathAbs(entry - sl) * S2_InpRRRatio)
               : entry - (MathAbs(entry - sl) * S2_InpRRRatio);
   ...
   req.symbol       = _Symbol;
   req.magic        = S2_InpMagic;
   req.volume       = lots;
   req.type         = (direction == 1) ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
   req.price        = entry;
   req.sl           = sl;
   req.tp           = tp;
   req.deviation    = S2_InpSlippage;
   req.comment      = "System2BreakoutRetest";
   ...
   if(res.retcode == TRADE_RETCODE_DONE || res.retcode == TRADE_RETCODE_PLACED)
   {
      g_cntTradesOpened++;
      g_tradesToday++;
      if(S2_InpCooldownBars > 0)
         g_cooldownBarsLeft = S2_InpCooldownBars;
      return true;
   }
...
bool HasOpenPosition()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if((int)PositionGetInteger(POSITION_MAGIC) != S2_InpMagic) continue;
      return true;
   }
   return false;
}
...
bool CheckRiskLimits()
{
   ...
   if(S2_InpProfitTargetPct > 0.0 && totalProfitPct >= S2_InpProfitTargetPct)
   {
      CloseAll();
      if(S2_InpPermanentHalt) g_permanentHalt = true;
      g_cntRejectedDrawdown++;
      return false;
   }

   if(totalDrawPct >= S2_InpMaxTotalLossPct)
   {
      CloseAll();
      if(S2_InpPermanentHalt) g_permanentHalt = true;
      g_cntRejectedDrawdown++;
   ...
   if(dailyDrawPct >= S2_InpMaxDailyLossPct)
   {
      if(!g_haltedToday)
      {
         CloseAll();
...
{
   Print("================ SYSTEM 2 BREAKOUT RETEST DIAGNOSTICS ================");
